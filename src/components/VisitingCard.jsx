...
const ACTIONS = [
  { key: 'save', label: 'Save Contact', hint: 'Add to your phone', icon: FiDownloadCloud, onClick: saveContact, primary: true },
  { key: 'whatsapp', label: 'WhatsApp', hint: 'Chat with us now', icon: FaWhatsapp, href: CONTACT.whatsappHref, external: true },
  { key: 'call', label: 'Call', hint: CONTACT.phone, icon: FiPhone, href: CONTACT.phoneHref },
  { key: 'email', label: 'Email', hint: CONTACT.email, icon: FiMail, href: CONTACT.emailHref },
  { key: 'instagram', label: 'Instagram', hint: '@webneststudio112026', icon: FiInstagram, href: CONTACT.instagramHref, external: true },
]

/**
 * The WebNest Studio digital business card panel. Shared by the standalone
 * /card page and the in-site modal (Navbar). Pass `onClose` to render a close
 * button and to dismiss the modal when an internal link is followed.
 */
export default function VisitingCard({ onClose }) {
  ...
  return (
    <div className="relative overflow-hidden rounded-3xl border border-gold-300/25 bg-ink-950/95 text-white shadow-2xl shadow-black/50 backdrop-blur">
      {onClose && (
        <button
          type="button"
          onClick={onClose}
          aria-label="Close"
          className="absolute right-3 top-3 z-10 flex h-8 w-8 items-center justify-center rounded-full border border-white/15 bg-ink-950/60 text-ink-200 transition-colors hover:border-gold-300/50 hover:text-gold-200"
        >
      ...
      <div className="relative border-b border-gold-300/15 px-6 pb-7 pt-8 text-center">
        ...
        <p className="mt-4 font-display text-2xl font-extrabold tracking-[0.14em]">
            ...
            <>
              <span
                className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl ${
                  action.primary ? 'bg-ink-950/20 text-ink-950' : 'bg-gold-400/10 text-gold-300'
                }`}
              >
      ...
      <div className="border-t border-white/10 px-5 py-4 text-center">
        <Link
          to="/"
          onClick={onClose}
          className="text-[0.68rem] text-ink-400 transition-colors hover:text-gold-300"
        >
          webneststudio.co.in
        </Link>
      </div>
