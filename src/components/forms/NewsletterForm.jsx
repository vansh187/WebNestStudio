    ...
    <form onSubmit={handleSubmit(onSubmit)} className="w-full space-y-2">
      <div className="flex flex-col gap-2 min-[420px]:flex-row">
        <input
          ...
          {...register('email')}
          className="min-w-0 flex-1 rounded-xl border border-ink-200 bg-white/40 px-4 py-3 text-sm text-ink-900 placeholder:text-ink-400 focus:border-gold-400 focus:outline-none focus:ring-2 focus:ring-gold-400/30 dark:border-ink-700 dark:bg-white/5 dark:text-white"
        />
        <button
          type="submit"
          disabled={isSubmitting}
          className="inline-flex min-h-12 shrink-0 items-center justify-center rounded-xl bg-ink-900 px-5 py-3 text-sm font-semibold text-white transition-transform hover:scale-[1.02] disabled:opacity-60 dark:bg-gold-400 dark:text-ink-950"
        >
