...
export default function SharedSnippet() {
  ...
  useEffect(() => {
    ...
    getShare(shareId)
      ...
      .catch((err) => {
        ...
        else setError(getErrorDetail(err, 'Could not load this shared CodeLab snippet.'))
      })
    return () => {
      cancelled = true
    }
  ...
  if (snapshot === null) {
    return (
      <NotFoundState
        title="Shared CodeLab snippet not found"
  ...
  return (
    <div className="mx-auto w-full max-w-[1600px] px-4 py-8 sm:px-6 lg:px-8">
      <BackButton fallback="/codelab" className="mb-2" />
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-widest text-gold-500">Shared CodeLab code</p>
          <h1 className="mt-1 font-display text-2xl font-bold text-ink-900 dark:text-white">
            {snapshot.title || 'Untitled snippet'}
          </h1>
          <p className="mt-1 text-sm text-ink-500 dark:text-ink-300">
            {lang?.label ?? normalizedLanguage}
        ...
        >
          <FiExternalLink className="h-4 w-4" /> Open in CodeLab
        </button>
      </div>
